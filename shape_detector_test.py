...
class MockArduino:
    ...
    def write(self, data):
        print(f"Mock Arduino received: {data.decode()}")
        self.processing = True
        self.start_time = time.time()
        # Simulate Arduino processing messages
        self.messages = ["detected - processing", "DONE"]
        self.in_waiting = len(self.messages)
...
print("Using mock Arduino for testing")
arduino = MockArduino()

# State management with proper timeouts
...
def send_to_arduino_simple(shape):
    ...
    try:
        ...
        # Set appropriate timeout based on your Arduino timing (reduced for testing)
        if shape == "circle":
            processing_time = 3  # Reduced from 10 for testing
        else:  # triangle or square
            processing_time = 5  # Reduced from 20 for testing
...
def initialize_camera():
    ...
    backends = [cv2.CAP_AVFOUNDATION, cv2.CAP_ANY]  # AVFoundation for macOS
...
if cap is None:
    print("Camera initialization failed - creating synthetic test data")
    cap = None

print("=== SYNC-OPTIMIZED SHAPE DETECTION WITH MOCK ARDUINO (MAC TEST) ===")
print("Instructions:")
print("- If you have a camera, it will use live video")
print("- If no camera, it will generate synthetic shapes for testing")
print("- Press 'r' to reset Arduino state")
print("- Press 'q' or ESC to exit")
print("- Draw shapes on paper or use objects to test detection")
...
def create_synthetic_frame():
    """Create a synthetic frame with a shape for testing when no camera is available"""
    frame = np.ones((480, 640, 3), dtype=np.uint8) * 50  # Dark background

    shapes = ['circle', 'square', 'triangle']
    shape_type = shapes[frame_count // 60 % len(shapes)]  # Change shape every 2 seconds
    ...
    if shape_type == 'circle':
        ...
        cv2.rectangle(frame, (center_x-60, center_y-60), (center_x+60, center_y+60), (255, 255, 255), -1)
    elif shape_type == 'triangle':
        pts = np.array([[center_x, center_y-80], [center_x-70, center_y+60], [center_x+70, center_y+60]], np.int32)
...
try:
    while True:
        ...
        if not arduino_ready:
            ...
            cv2.putText(frame, "MOCK ARDUINO READY - DETECTING SHAPES", (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            ...
            if current_time - last_detection_time > 1.0:
                ...
                if valid_contours:
                    ...
                    if shape in ("circle", "triangle", "square"):
                        ...
                        # Unknown shape
                        cv2.putText(frame, "Unknown shape detected", (50, 100),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)

                last_detection_time = current_time

        # Add test mode indicator
        mode_text = "LIVE CAMERA" if cap is not None else "SYNTHETIC TEST MODE"
        cv2.putText(frame, mode_text, (10, frame.shape[0] - 20),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)

        # Display frame
        cv2.imshow("Robot Shape Sorting System - Mac Test", frame)
